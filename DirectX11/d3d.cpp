...
// I could have simplified it and taken out some items but it is probably better to get all of this covered in a 
// single tutorial dedicated to it.
// The screenWidth and screenHeight variables that are given to this function are the widthand height of the window 
// we created in the SystemClass.
...
// The fullscreen variable is whether we are running in windowed mode or fullscreen. Direct3D needs this as well for 
						  ...
						  float screenDepth, float screenNear)
{
	...
	// and it will calculate the proper refresh rate. If we don't do this and just set the refresh rate to 
	// a default value which may not exist on all computers then DirectX will respond by performing a blit 
	// instead of a buffer flip which will degrade performance and give us annoying errors in the debug output.
	...
	// Now that we have stored the numeratorand denominator for the refresh rateand the video card information we can 
	...
	// If vsync is set to true in our graphicsclass.h header then this will lock the refresh rate to the 
	// system settings (for example 60hz). That means it will only draw the screen 60 times a second 
	// (or higher if the system refresh rate is more than 60). However if we set vsync to false then it will 
	// draw the screen as many times a second as it can, however this can cause some visual artifacts.
	...
	// You can set this to 10 or 9 to use a lower level version of DirectX if you plan on supporting multiple versions 
	// or running on lower end hardware.

	// Set the feature level to DirectX 11.
	featureLevel = D3D_FEATURE_LEVEL_11_0;

	// Now that the swap chain description and feature level have been filled out we can create the swap chain, the Direct3D device, 
	// and the Direct3D device context. The Direct3D device and Direct3D device context are very important, they are the interface 
	// to all of the Direct3D functions. We will use the device and device context for almost everything from this point forward.
	// Those of you reading this who are familiar with the previous versions of DirectX will recognize the Direct3D device but will be 
	// unfamiliar with the new Direct3D device context. Basically they took the functionality of the Direct3D device and split it up into 
	// two different devices so you need to use both now.
	...
	// Also if you are testing DirectX 11 functionality yourself and don't have a DirectX 11 video card then you can replace 
	// D3D_DRIVER_TYPE_HARDWARE with D3D_DRIVER_TYPE_REFERENCE and DirectX will use your CPU to draw instead of the video card hardware. 
	...
	// Also some hybrid graphics cards work that way with the primary being the low power Intel card and the secondary being the high 
	// power Nvidia card. To get around this you will need to not use the default deviceand instead enumerate all the video cards in the 
	// machine and have the user choose which one to useand then specify that card when creating the device.
	...
	// Now that we have a swap chain we need to get a pointer to the back bufferand then attach it to the swap chain.
	...
	// This will bind the render target view and the depth stencil buffer to the output render pipeline. This way the graphics that the 
	...
	// Now that the render targets are setup we can continue on to some extra functions that will give us more control over our scenes 
	...
	// By default DirectX already has a rasterizer state set up and working the exact same as the one below but you have no control to 
	// change it unless you set up one yourself.
