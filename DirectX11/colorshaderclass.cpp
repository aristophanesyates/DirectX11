...
bool ColorShaderClass::Initialize(ID3D11Device* device, HWND hwnd)
{
	...
	std::copy(std::begin(L"vs_color.hlsl"), std::end(L"vs_color.hlsl"), vs_filepath);
	std::copy(std::begin(L"ps_color.hlsl"), std::end(L"ps_color.hlsl"), ps_filepath);
...
bool ColorShaderClass::InitializeShader(ID3D11Device* device, HWND hwnd, WCHAR* vsFilename, WCHAR* psFilename)
{
	...
	// We give it the name of the shader file, the name of the shader, the shader version (5.0 in DirectX 11), 
	// and the buffer to compile the shader into. 
	// If it fails compiling the shader it will put an error message inside the errorMessage string which we send to another function 
	// to write out the error. If it still fails and there is no errorMessage string then it means it could not find the shader file in which 
	...
	// Once the vertex shader and pixel shader code has successfully compiled into buffers, we then use those buffers to create the 
	...
	// For this layout we are telling it the first 12 bytes are position and the next 16 bytes will be color, AlignedByteOffset shows where each
	// element begins. 
	...
	// As you saw in the vertex shader we currently have just one constant buffer so we only need to setup one here 
	...
	// Once we fill out the description we can then create the constant buffer interface and then use that to access the internal variables
